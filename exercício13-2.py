...
def main():
    ...
    parser.add_argument(
        "source_file", help="The text file containing the source and target urls.")
    parser.add_argument(
        "data_struct", help="Select 'list' or 'dict to use in checking for words.")
    parser.add_argument(
        "-c", "--count", help="Print out specified number of most frequently used words", type=int)
    ...
    if data_struct not in ['list', 'dict']:
        ...
        word_list_file = open(
            'words_list', 'rb')
        complete_word_set = pickle.load("word_list_file")
    elif data_struct == 'dict':
        word_list_dict_file = open(
            'words_dict', 'rb')
        complete_word_set = pickle.load("word_list_dict_file")
    ...
    for line in f:
        ...
        for word in line.lower().strip().split():
            count_word(word, complete_word_set,
                       book_word_dict, unknown_word_dict)
    ...
    sorted_word_dict = collections.OrderedDict(
        sorted(book_word_dict.items(), reverse=True, key=lambda t: t[1]))
    sorted_unknown_word_dict = collections.OrderedDict(
        sorted(unknown_word_dict.items(), reverse=True, key=lambda t: t[1]))
